...
def parse_comm(s):
    ...
    for i in range(len(s)):
        ...
        if l == "[":
            ...
            a = " ".join([parse_comm(x) for x in out]) + s[start: i]
            b = s[i+1:len(s)]
            a, b = parse_comm(a), parse_comm(b)
            if l == ",":
                comm_out = a + b + " "+ inverse(a) +" "+ inverse(b)
            if l == ":":
                comm_out = a + b + " " +inverse(a)
            return comm_out
    if len(out) == 0:
        return s
    else:
